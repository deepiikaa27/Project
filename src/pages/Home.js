...
import Testimoni from "../components/UI/Slider/Testimoni.js";
...
const Home = () => {
  ...
  useEffect(() => {
    const filterdPizza = data.filter((item) => item.category === "Pizza");
    const slicedPizza = filterdPizza.slice(0, 4);
  ...
  useEffect(() => {
    ...
    if (category === "Burger") {
      const filterProduct = data.filter((item) => item.category === "Burger");
      setProducts(filterProduct);
    }
    if (category === "Pizza") {
      const filterProduct = data.filter((item) => item.category === "Pizza");
      setProducts(filterProduct);
    }
    if (category === "Bread") {
      const filterProduct = data.filter((item) => item.category === "Bread");
      setProducts(filterProduct);
    }
  }, [category]);
  return (
